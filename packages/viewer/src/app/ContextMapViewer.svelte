...
<script lang="ts">
  ...
  import ContextMapApp from "../ContextMapApp.svelte";
  ...
  async function loadSettings(spec: Settings) {
    ...
    try {
      let projectionColumns: { x: string; y: string } | undefined;
    ...
    >
      {#if stage == "load-data"}
        <div class="w-[40rem] flex flex-col gap-2">
          <div class="text-2xl font-bold text-center mb-4">ContextMap</div>
          <div class="text-sm text-slate-600 dark:text-slate-400 text-center mb-4">
            Visual Context Management for AI
          </div>
          ...
          {/if}
          <div class="text-slate-400 dark:text-slate-500 mt-4 text-sm text-center">
